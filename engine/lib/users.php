<?php
		...
		implements Friendable
	{
		...
		public function save()
		{
			...
			return create_user_entity($this->get('guid'), $this->get('name'), $this->get('username'), $this->get('password'), $this->get('salt'), $this->get('email'), $this->get('language'), $this->get('code'), $this->get('Sexo'), $this->get('Barrio'));
		}
	 ...
	 * @param string $url
	 */
	function create_user_entity($guid, $name, $username, $password, $salt, $email, $language, $code, $sexo, $barrio)
	{
		global $CONFIG;
		...
		$sexo = sanitise_string($sexo);
		$barrio = sanitise_string($barrio);
		...
		if ($row)
		{
			...
			if ($exists = get_data_row("SELECT guid from {$CONFIG->dbprefix}users_entity where guid = {$guid}")) {
				$result = update_data("UPDATE {$CONFIG->dbprefix}users_entity set name='$name', username='$username', password='$password', salt='$salt', email='$email', language='$language', code='$code', Sexo='$sexo', Barrio='$barrio', last_action = ". time() ." where guid = {$guid}");
				if ($result != false)
				{
			...
			else
			{
				...
				$result = insert_data("INSERT into {$CONFIG->dbprefix}users_entity (guid, name, username, password, salt, email, language, code, Sexo, Barrio) values ($guid, '$name', '$username', '$password', '$salt', '$email', '$language', '$code', '$sexo', '$barrio')");
				if ($result!==false) {
					$entity = get_entity($guid);
