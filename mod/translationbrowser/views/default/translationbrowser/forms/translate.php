...
//	print_r($data);
	...
	$body .= "<input type='radio' name='export[]' value='downloadallinzip' />" . elgg_echo('translationbrowser:downloadallinzip');
	$body .= "</div>";
	...
	foreach($data->lang_trans as $key => $word)
	{
		...
		$body .= "<textarea rel='words[{$key}]' cols='50' rows='2'>{$word_to_trans}</textarea>";
	}
	
	$body .= "<div class='cont-button'>" . elgg_view('input/submit', array('value' => elgg_echo('translationbrowser:translate'))) . "</div>";
	
	echo "<div id='cont-words'>";
	echo  elgg_view('input/form', array('internalid' => 'browsertranslate', 'internalname' => 'browsertranslate', 'action' => "{$vars['url']}action/translationbrowser/translate", 'body' => $body));
	echo "</div>";
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#cont-words textarea').focus(function(){
			oObject = $(this);
			oObject.attr('name', oObject.attr('rel'));
		})
