<?php
	/**
	 * override of elgg string_to_tag_array(),also split by '，' and ' '.
	 ...
	 */
	function string_to_tag_array_snowMod($string) {

		if (is_string($string)) {
			#start pyb mod by snow
			//conver '，' and ',' to ' '
	 ...
	 * 
	 */
	function friendlyforward($params = ''){
		$to = explode('?',$_SERVER['HTTP_REFERER']);
		$to = $to[0];
     ...
     **/
    function parse_urls_snowMod($text) {
       	...
       	create_function(
            ...
            '
            	...
            	return "<a href=\"$url\" style=\"text-decoration:underline;\" target=\"_blank\">$urltext</a>";
            '
        ), $text);
    }
	/**
	 ...
	 * @param string $_Code
	 * @return string
	 * @author http://afericazebra.blog.163.com
	 */
	function Pinyin($_String, $_Code = 'utf8') {
		...
		/*
		 * start mod by snow
		 * no need to strip non-eng char,i think.  
		 */
		return $_Res;
