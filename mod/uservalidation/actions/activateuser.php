<?php
	...
	if (($user) && ($user instanceof ElggUser)) {
		...
		$user->enable();
		trigger_plugin_hook('activateuser:postprocess','user',$user);
		system_message(elgg_echo('uservalidation:admin:confirm:success'));
		notify_user($user->guid, $CONFIG->site->guid, sprintf(elgg_echo('uservalidation:success:subject', $user->language), $user->name), sprintf(elgg_echo('uservalidation:success:body', $user->language), $user->name, $CONFIG->site->name, $CONFIG->site->url), NULL, 'email');
